...
CREATE TABLE IF NOT EXISTS `form_exemplo`.`acesso_menu` (
  ...
  `img_menu` VARCHAR(45) NULL COMMENT 'imagem para o item de menu',
  `imgdisabled` VARCHAR(45) NULL,
  `dissabled` TINYINT NULL,
  `hotkey` VARCHAR(45) NULL,
  `separator` VARCHAR(45) NULL,
  `sit_ativo` VARCHAR(1) NOT NULL DEFAULT 'S' COMMENT 'Informa se o registro está ativo ou não',
  `dat_inclusao` DATETIME NOT NULL DEFAULT NOW(),
...
-- -----------------------------------------------------
-- Placeholder table for view `form_exemplo`.`user_menu`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `form_exemplo`.`user_menu` (`iduser` INT, `login_user` INT, `idperfil` INT, `nom_perfil` INT, `idmenu` INT, `nom_menu` INT);

-- -----------------------------------------------------
-- View `form_exemplo`.`user_menu`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `form_exemplo`.`user_menu`;
USE `form_exemplo`;
CREATE  OR REPLACE VIEW `user_menu` AS
select 
 u.iduser
,u.login_user
,p.idperfil
